...
import static au.org.raid.api.service.stub.InMemoryStubTestData.NONEXISTENT_TEST_NOMINATIM_OPENSTREETMAP_URI;
import static au.org.raid.api.service.stub.InMemoryStubTestData.SERVER_ERROR_TEST_OPENSTREETMAP_URI;
import static au.org.raid.api.util.ObjectUtil.areEqual;

@Slf4j
public class NominatimOpenStreetMapValidatorStub extends NominatimOpenStreetMapUriValidator {
    private final Long delayMilliseconds;
    public NominatimOpenStreetMapValidatorStub(final Long delayMilliseconds) {
        super(null);
    ...
    public List<ValidationFailure> validate(String uri, String fieldId) {
        ...
        if (!uri.matches(regex)) {
            ...
            if (areEqual(uri, NONEXISTENT_TEST_NOMINATIM_OPENSTREETMAP_URI)) {
                failures.add(new ValidationFailure()
                        .fieldId(fieldId)
                        .errorType(INVALID_VALUE_TYPE)
                        .message(URI_DOES_NOT_EXIST)
                );
            } else if (areEqual(uri, SERVER_ERROR_TEST_OPENSTREETMAP_URI)) {
                failures.add(new ValidationFailure()
                        .fieldId(fieldId)
                        .errorType(INVALID_VALUE_TYPE)
