...
# Output file
OUTPUT_FILE="combined_handles.json"
...
fetch_backend() {
  ...
  echo "Response from $backend (first 100 chars): ${response:0:100}"
  
  # Process JSON array directly
  echo "$response" | jq -r '.[]' 2>/dev/null | while read -r handle; do
    if [ -n "$handle" ]; then
      # Debug: Show what's being processed
      echo "Adding handle: $handle from $backend"
      
      # Add to result with backend as value
...
fetch_backend "prod" "https://static.prod.raid.org.au/raids/handles.json"
fetch_backend "stage" "https://static.stage.raid.org.au/raids/handles.json"
fetch_backend "demo" "https://static.demo.raid.org.au/raids/handles.json"
fetch_backend "test" "https://static.test.raid.org.au/raids/handles.json"
